...
static const FnThis    ItemCtor   = (FnThis)   0x1A340;   // MenuOptionItem ctor (0x84 bytes)
static const FnThisU32 SetLabel   = (FnThisU32)0x19E70;   // item+0x7A (u16 string index)
static const FnThisU32 SetValue   = (FnThisU32)0x19E80;   // item+0x7C (u8 string index)
static const FnThisU32 SetAlign   = (FnThisU32)0x19E90;   // item+0x70
static const FnThisU32 SetScale   = (FnThisU32)0x1A320;   // item+0x68
static const FnInputT  InputTest  = (FnInputT) 0x13470;   // (id, pad) -> pressed this frame
...
static void __fastcall Hk_Build(uint32_t self, uint32_t edx) {
    Orig_Build(self, edx);            // ...which tail-jumps through vtable+0x14 = Hk_Enter
    BuildRow(self);
...
static uint32_t __fastcall Hk_Update(uint32_t self, uint32_t edx) {
    ...
    uint32_t r = Orig_Update(self, edx);
...
int InstallMeatUi() {
    ...
    *U32(kVTable + 0x08) = (uint32_t)(uintptr_t)&Hk_Build;
    *U32(kVTable + 0x10) = (uint32_t)(uintptr_t)&Hk_Update;
    *U32(kVTable + 0x14) = (uint32_t)(uintptr_t)&Hk_Enter;
